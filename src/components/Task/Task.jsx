        ...
        return(
            <div className="task" >
                ...
                <div className="task-controls">
                    {this.props.status !== 'planned' && this.props.status !== 'complete' && <button className="task-control control-run" onClick={() => this.handle(this.props.id, 'planned')}>To Planned</button>}
                    {this.props.status !== 'work' && <button className="task-control control-run" onClick={() => this.handle(this.props.id, 'work')}>To work</button>}
                    {this.props.status !== 'complete' && this.props.status !== 'planned' && <button className="task-control control-run" onClick={() => this.handle(this.props.id, 'complete')}>Completed</button>}
                    <button className="task-control control-del" onClick={() => this.handle(this.props.id, 'delete')}>Delete</button>
                </div>
            </div>
        )
    }
